# reference from https://github.com/platisd/duplicate-code-detection-tool
'''
...
def main():
    ...
    for source_file in source_code:
        ...
        print("%s %s" % (file_column_label.center(largest_string_length), similarity_column_label))
        print("-" * (largest_string_length + similarity_label_length))
        for similarity, source in zip(sims[query_doc_tf_idf], source_code):
			...
			# ignore less than 10% of similarity
            if similarity < 0.2:
                continue
			
            similarity_percentage = similarity * 100            

            print("%s     " % (source.ljust(largest_string_length)) + "%.2f" % (similarity_percentage))
